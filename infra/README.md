...
# Prod — yaml bakes VITE_ADMIN_API_URL; pass GIS client from GSM (never commit)
...
`GET /live/events` `bulk_process` snapshots are commit-triggered, not polled per connection. Migration [`20260827230000_drop_bulk_stats_notify.sql`](../db/migrations/20260827230000_drop_bulk_stats_notify.sql) installs trigger `drop_bulk_process_stats_notify` (`AFTER INSERT OR UPDATE` on `drop_bulk_process_stats`) → `pg_notify('drop_bulk_stats_changed', download_id::text)` — payload is the id only, never PII, and NOTIFY delivers only after COMMIT. Postgres dedupes identical NOTIFY payloads within a transaction, so multi-UPDATE commits collapse to one notification. admin-api bridges LISTEN → SSE via `admin_api.live_rollup_notify.BulkRollupBroadcaster` (dedicated asyncpg connection, started/stopped with the app lifespan): reconnect with backoff plus catch-up refresh, coalesce window `LIVE_BULK_NOTIFY_COALESCE_MS` (default `200`), immediate flush when a batch's matching completes, per-id dedupe; each flush re-reads the rollup so payloads stay exact. Backstop: one shared 5s poll of the latest process per admin-api process keeps clients exact if LISTEN drops; the legacy per-connection bulk poll remains only as an ImportError fallback when the bridge module is absent. The per-connection `matching_progress` 1.5s poll and all event shapes are unchanged — web clients need no changes.
...
**Admin-api-prod snapshot hang (00078):** Deploy with `--no-traffic` and a **new** tag — do not reuse hanging `admin-api-prod-00078` / `superadmin-verts` if that image is the hanging collector. Confirm on the new revision (direct revision URL): `GET /ops/drop/console/snapshot` 200 **with bytes in < 8s**, `GET /me` 200 **< 2s**, `GET /ops/drop/pipeline/summary` 200. Only then shift collector traffic. If snapshot still hangs or returns 0 bytes, keep live **00077**. No `--to-latest`. This is an admin-api collector hang gate, not a web Architecture B gate — it does not pin nginx `/api` or refuse GIS.
...
### Cloud Run auth (Architecture B)

**Architecture B is the intended authorized identity:** admin-api is the **resource server** (Cloud Run IAP **off**; app-level `REQUIRE_IAP_IDENTITY=true`; `IAP_OAUTH_CLIENT_ID` already set; CORS already includes admin-web-prod). Humans use **admin-web IAP** as the SSO front door. Browser JSON uses a Google Identity Services user ID token (`Authorization: Bearer`, `aud` = OAuth client `IAP_OAUTH_CLIENT_ID`). Master yaml was reverted in `2094211`; ARCH-B is re-shipping the bake. **Bake ≠ live traffic** until that cutover — do not invent a traffic percent. Server-Sent Events stay same-origin `/api/live/events` (nginx mints a service-account Bearer and forwards IAP headers); empty-VITE is local/emergency rollback, not a standing prod JSON path. CLI: `habeas-cli auth login --adc` or `auth login` — pin both `ADMIN_API_ID_TOKEN_AUDIENCE` and `IAP_OAUTH_CLIENT_ID` (unchanged). Live `admin-api-dev` IAM already has `allUsers` + compute SA + `jsirven@` — that is how DEV B works; copy that invoker set to `admin-api-prod`. GIS JWT `aud` is the OAuth client, not the Cloud Run URL, so Cloud Run IAM needs `allUsers` on admin-api for the browser GIS Bearer to reach the service; app-level `REQUIRE_IAP_IDENTITY` still requires a verified Bearer. Workers never get `allUsers`. Cloud Build admin-api yamls: `--no-allow-unauthenticated`, `--no-iap`, `REQUIRE_IAP_IDENTITY=true`. `resolve_actor` rejects IAP email header alone (needs verified Bearer; header-alone → 401). Do **not** re-enable Cloud Run IAP on admin-api. Do **not** re-run [`admin-api-dev-iam.yaml`](cloudbuild/admin-api-dev-iam.yaml) (turns IAP on). Do **not** edit accepted SirvenOS architecture decision records from this repo.
...
| Surface | Invoker |
|---------|---------|
| `admin-api-dev` | Live IAM: `allUsers` + compute SA + `jsirven@` (how DEV B works; copy this invoker set to `admin-api-prod`). Cloud Run **IAP off** (`--no-iap`); app-level `REQUIRE_IAP_IDENTITY` requires a verified Bearer (`user_jwt` / `bearer_jwt` / `iap_header` = Bearer **plus** IAP email). Header-alone → 401 |
| `admin-web-dev` / `ops-ia-web-dev` | Public Cloud Run + browser IAP front door (see `admin-web-dev.yaml` / `ops-ia-web-dev.yaml`). Architecture B front door is **admin-web-dev** (GIS JSON → admin-api). `ops-ia-web-dev` still proxies same-origin `/api` for Server-Sent Events — not a standing prod JSON path |

| Workers (`drop-connector-dev`, `drop-ingestor-dev`, `request-dispatcher-dev`, `data-fulfillment-dispatcher-dev`, `data-vertical-matching-dev`, `hash-index-refresh-dev`) | Runtime SA of admin-api only (`95660886550-compute@developer.gserviceaccount.com`) — never user/IAP direct, never `allUsers` |
...
| Browser SSO | admin-web IAP front door (human page access). Architecture B API session is a Google Identity Services user Bearer. Server-Sent Events stay on the `/api` proxy (nginx + SA Bearer). Empty `VITE_ADMIN_API_URL` rebuild is local/emergency rollback only — not a standing prod JSON path |
...
| `roles/run.invoker` | `allUsers` + compute SA + `jsirven@` (live on `admin-api-dev`; copy to `admin-api-prod` so GIS can pass Cloud Run IAM). Workers never get `allUsers`. | `admin-api-dev` / `admin-api-prod` (Cloud Run IAP **off**; app-layer `REQUIRE_IAP_IDENTITY` requires a verified Bearer) |
...
2. **Architecture B is the intended authorized identity:** admin-api is the resource server; browser JSON uses a Google Identity Services user-delegated Google ID token. IAP stays on admin-web for humans. Master yaml was reverted in `2094211`; ARCH-B is re-shipping the bake. **Bake ≠ live traffic** until that cutover — do not invent a traffic percent. nginx `/api` stays for Server-Sent Events (`/api/live/events`) and empty-`VITE_ADMIN_API_URL` local/emergency rollback — not a standing prod JSON path. Live `admin-api-dev` IAM already has `allUsers` + compute SA + `jsirven@`; copy that to `admin-api-prod`. Workers never get `allUsers`. CLI ADC + IAP login unchanged. Cookie IAP (`credentials: 'include'`) is not the B API session.
...
**admin-api-prod:** `_MATCHING_URL` → `data-vertical-matching-prod` (not legacy `matching-prod`). Cloud Run scaling: `max-instances=3`, `min-instances=1` (see [`cloudbuild/admin-api-prod.yaml`](cloudbuild/admin-api-prod.yaml)). Cloud Run IAP **off**, `REQUIRE_IAP_IDENTITY=true`, `IAP_OAUTH_CLIENT_ID` already set, CORS already includes admin-web-prod.

**Architecture B is the intended authorized identity:** admin-web IAP is the human SSO front door; browser JSON uses a GIS user Bearer against admin-api-prod (`admin-web-prod.yaml` bakes `VITE_ADMIN_API_URL=https://admin-api-prod-hsa55rg7ja-uk.a.run.app`; `VITE_GOOGLE_CLIENT_ID` from GSM `iap-oauth-client-id` at submit — never commit). Master yaml was reverted in `2094211`; ARCH-B is re-shipping the bake. **Bake ≠ live traffic** until that cutover — do not invent a traffic percent. nginx `/api` stays for SSE and local/emergency rollback — not a standing prod JSON path. Copy live `admin-api-dev` IAM (`allUsers` + compute SA + `jsirven@`) to `admin-api-prod`. Workers never get `allUsers`. CLI ADC / IAP login unchanged.
