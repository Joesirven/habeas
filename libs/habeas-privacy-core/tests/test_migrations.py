...
def test_auth0_vertical_matching_stats_migration_exists():
    ...
    assert migration.exists()
    content = migration.read_text()
    assert "core_auth0_attempts_vertical_stats" in content
    assert "core_drop_bulk_vertical_auth0_matching_delta" in content
    assert "auth0_attempts_vertical_stats" in content
    assert "drr.list_type = 'Email'" in content
    assert "request_vertical_matching" in content
    assert "migrate:up" in content
    assert "migrate:down" in content
