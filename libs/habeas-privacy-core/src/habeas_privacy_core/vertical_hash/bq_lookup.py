"""Look up Auth0 vendor_record_id values from BigQuery ``external_hash_index``.
...
class Auth0HashLookupError(Exception):
...
def lookup_auth0_vendor_ids_by_email_hash(
    ...
    *,
    ...
    fq_table = f"`{project_id}.{dataset_id}.{AUTH0_EMAIL_HASH_BUILD_TABLE}`"
    sql = f"""
        SELECT CAST(vendor_record_id AS STRING) AS vendor_record_id
          FROM {fq_table}
         WHERE hash_value = @hash_value
           AND system = '{AUTH0_SYSTEM}'
    """
    ...
    try:
        ...
        rows = list(result)
    except Auth0HashLookupError:
        raise
    except Exception as exc:
        message = redact_error_text(str(exc))
        logger.error(
            "auth0 hash lookup failed",
        ...
        raise Auth0HashLookupError(message, retry_seconds=retry_seconds) from None
...
def _query_job_config(hash_value: str) -> Any:
    try:
        ...
        raise Auth0HashLookupError("google-cloud-bigquery is not installed") from exc
...
def _default_client() -> Any:
    try:
        ...
        raise Auth0HashLookupError("google-cloud-bigquery is not installed") from None
