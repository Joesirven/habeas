...
function matchingProgressCount(
  ...
  const top = progress[status]
  if (typeof top === 'number') return top
  const row = progress.by_status?.find((item) => item.status === status)
...
async function loadBatchRequestRuns(params: {
  ...
  // Snapshot miss: list only. Filter changes: cheap lite summaries (ledger, no spine).
...
function BatchProcessExpandRow({
  ...
  const collapsed = collapsedBulkRowDisplay({
    process_at: row.process_at,
    intake_source: row.intake_source,
    label: row.label,
    download_status: detail?.download_status ?? row.download_status,
    overall: detail?.overall ?? row.overall,
    request_rows: detail?.request_rows ?? row.request_rows,
  })
  const processName = collapsed.dateLabel
  const status = collapsed.statusLabel
            ...
            <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5">
              ...
              <span className="ml-auto flex shrink-0 items-center gap-2 text-[0.6rem] tabular-nums text-mute">
                ...
                {expanded ? (
                  ...
                  <span>
                    Prog {collapsed.progressLabel || '—'}
                    {collapsed.countLabel ? ` · ${collapsed.countLabel}` : ''}
                  </span>
                )}
              </span>
            </div>
  ...
  const listQuery = useQuery({
    ...
    queryFn: () =>
      loadBatchRequestRuns({
        ...
        intake,
        downloadStatus,
        includeSummary: !(defaultFilters && snapshotError),
      }),
...
export function DropPipelinePageInner() {
  ...
  const caSchedule = summaryData?.ca_drop_schedule
  ...
  const matchDrainActive = matchingProgress?.drain?.active ?? null
  const matchRate =
