...
import {
  approveNoticeReview,
  listNoticeReviewApprovals,
  type ApprovalRecord,
} from '@/lib/api'

function NoticeReviewTableSkeleton({ rows = 5 }: { rows?: number }) {
  return (
...
export function NoticeReviewPage() {
  const queryClient = useQueryClient()
  const approvalsQuery = useQuery({
    queryKey: ['admin-api', 'approvals', 'notice.review'],
    queryFn: () => listNoticeReviewApprovals('pending'),
  ...
  return (
    <section className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-white">Notice review</h2>
        <p className="mt-2 max-w-2xl text-slate-400">
          Approve notice.review gates before weekly DROP response upload can proceed.
        </p>
      </div>

      <div className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900/60">
        {approvalsQuery.isPending && <NoticeReviewTableSkeleton />}
        {approvalsQuery.isError && (
          <p className="p-6 text-red-300">Could not load notice.review approvals.</p>
        )}
        {approvalsQuery.isSuccess && approvalsQuery.data.length === 0 && (
          <p className="p-6 text-slate-400">No pending notice.review items.</p>
        )}
        {approvalsQuery.isSuccess && approvalsQuery.data.length > 0 && (
