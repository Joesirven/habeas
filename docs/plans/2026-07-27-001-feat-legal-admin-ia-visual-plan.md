...
| KTD3 storage of calculated/`due_at` + override stamps **on `requests`** | **Superseded (2026-07-30)** — product KD35/R35 due model remains; storage is append-only `request_due_overrides` + derive-on-read, close via `request_closures` (plan `2026-07-30-001`). Spine stays insert-only. |
...
- KTD3. **SLA + `due_at` schema** — ~~new migration adds `requests.due_at timestamptz`, `requests.due_at_override_at`, `requests.due_at_override_by`~~ **Superseded (storage, 2026-07-30):** do **not** store calculated `due_at` or override stamps on `requests` — see plan `2026-07-30-001-feat-requests-spine-immutability-plan.md` (append-only `request_due_overrides` + derive-on-read; close via `request_closures`). **Still in force:** `legal_sla_settings` single-row table for global stage durations (data owner review, legal/pre-fulfillment, fulfillment, lifecycle); product SLA behavior (KD35) unchanged. `sla_monitor` may read later; v1 calculates due on read (lifecycle from `received_at` + settings; stage clocks as pure functions) and persists **admin overrides only**.
