...
- Load missing BQ tables for full privacy-install parity (CEE_*, analytics_cyclic/frozen/retired, `production_datasets.occupation` / AVEV / applicant_typecodes)
...
flowchart TD
  ...
  FD -->|access| BQ[BigQuery reproduce]
  BQ --> GCS_A["GCS bulk-run/process/request/id"]
...
**BQ access table map (privacy install → `example-gcp-project`, probed 2026-07-21):**

Verdict: **not full script parity**. Core MDR tables for person/vote/district/phones/ballots/models are present; CEE_* and most ProductionDatasets tables are absent. MVP exports the Include row set and writes a manifest of included vs deferred.

| Privacy install source | BQ target | MVP | Notes |
|------------------------|-----------|-----|-------|
| `person_db.person` | `person_db.person` | Include | 228 cols present |
| `person_db.vote_history` | `person_db.vote_history` | Include | |
| `person_db.district` | `person_db.district` | Include | |
| `person_db.phones` | `person_db.phones` | Include | |
| `person_db.ballots` | `person_db.ballots` | Include | |
| `person_db.analytics_continuous` | `person_db.analytics_continuous` | Include | Thin (~7 cols) vs Vertica — export what exists |
| `person_db.models` | `person_db.models` (VIEW) | Include | ~47 cols |
| `person_db.analytics_cyclic` / `_frozen` / `_retired` | — | Deferred | Not in project |
| `person_db.cee_*` / `CEE_*` (person/household synthetics, voting behavior, commercial, …) | — | Deferred | Not in project |
| `ProductionDatasets.occupation` | — | Deferred | Missing |
| `ProductionDatasets.AVEV_ROLLUP_*` | — | Deferred | Missing |
| `ProductionDatasets.applicant_typecodes_2022` | — | Deferred | Missing |
| (extra in BQ, not script core) household / accession / beststate / election_summary / legacy_race_2_0 | same in `person_db` | Optional Include | Join on dwid/state when useful |
| `production_datasets.emails_digital_only_24q2` | same | Out of MVP unless Legal asks | Only table in that dataset today |
...
- Query allowlisted tables; write one file per table (TSV/CSV) + `manifest.json` (table list, row counts, generated_at — no raw PII in logs).
...
**Patterns to follow:** `app/matching/src/matching/bq_lookup.py` (parameterized SQL, redacted errors)
...
- Datasets: `person_db`, `production_datasets`, `drop_hash_index`, `drop_hash_experiment`
- **Present / MVP:** `person` (228 cols), `vote_history`, `district`, `phones`, `ballots`, `analytics_continuous` (thin ~7 cols), `models` (view ~47 cols); optional extras `household`, `accession`, `beststate`, `election_summary`, `legacy_race_2_0`
- **Missing vs script (deferred):** `analytics_cyclic` / `_frozen` / `_retired`; all `cee_*` / `CEE_*`; `ProductionDatasets.occupation`, `AVEV_ROLLUP_*`, `applicant_typecodes_2022`
- `production_datasets` today: only `emails_digital_only_24q2` (not a script core table for privacy FULL join)
