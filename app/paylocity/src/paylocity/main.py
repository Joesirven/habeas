...
app = FastAPI(title="Habeas Privacy Paylocity Worker", version="0.1.0", lifespan=lifespan)
...
async def _is_suppress_paylocity_approved(
    conn: asyncpg.Connection,
    request_id: str,
) -> bool:
    """Return True when suppress.paylocity has an approved approval_requests row."""
...
async def suppression_submit():
    ...
    Live suppress MUST respect approval rule ``suppress.paylocity`` (seeded in
    ``approval_rules``; requires ``data_owner.hr``). Do not call the Paylocity
    vendor until an approved ``approval_requests`` row exists for the request.
    """
    if not settings.database_url:
        raise HTTPException(status_code=503, detail="database not configured")
    ...
    pool = get_pool()
    async with pool.acquire() as conn:
        rule = await fetch_active_rule(conn, SUPPRESS_PAYLOCITY_ACTION)
        # Fail closed: missing rule or requires_approval → need approved row.
        requires_approval = True if rule is None else bool(rule.get("requires_approval"))
        if requires_approval:
            request_id = str(row["request_id"])
            if not await _is_suppress_paylocity_approved(conn, request_id):
...
async def hash_refresh_process():
    """Claim vertical_hash_refresh for Paylocity; extract → hash → BQ → dbt."""
    if not settings.database_url:
        raise HTTPException(status_code=503, detail="database not configured")
