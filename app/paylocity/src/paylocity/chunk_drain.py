...
def _mart_table_for_kind(kind: str) -> str | None:
    if kind == "email":
        return PAYLOCITY_EMAIL_HASH_BUILD_TABLE
    if kind == "phone":
        return PAYLOCITY_PHONE_HASH_BUILD_TABLE
    if kind == "ndz":
        return PAYLOCITY_NDZ_HASH_BUILD_TABLE
    return None
...
def _lookup_hashes_for_kind(
    ...
    *,
    ...
    if kind == "email":
        batch_lookup, _error_type = _resolve_batch_lookup()
        return batch_lookup(hash_values, client=bq_client)
    if kind == "phone":
        ...
        for kind, group in by_kind.items():
            if lookup_batch is None:
                try:
                    ...
                    safe = redact_error_text(str(exc))
                    logger.error(
                        "paylocity_drain_mart_check_error",
                        extra={
                            "event": "paylocity_drain_mart_check_error",
                            "list_kind": kind,
                            "error_summary": safe,
                        },
                    )
                if not mart_ok:
                        ...
                        outcomes.append(
                            {
                                ...
                                "audit_payload": _error_audit(
                                    error_code="paylocity_lookup_error",
                                    error_class="PaylocityHashLookupError",
                                    error_detail=(
                    ...
                    logger.info(
                        "paylocity_drain_mart_missing",
                        extra={
                            "event": "paylocity_drain_mart_missing",
                            "list_kind": kind,
                            "prepared": len(group),
                        },
                    )
            ...
            except PaylocityHashLookupError as exc:
                retry_after = datetime.now(UTC) + timedelta(
