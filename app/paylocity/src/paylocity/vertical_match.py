"""Paylocity worker matching — mart lookup + snapshot persist.

Loads the request's DROP hash for Email / Phone / NDZ (same field order as
matching/drop_hash ADR-21), looks up opaque vendor ids on the matching
Paylocity serving mart, and upserts ``request_vertical_matching``. Lookup
failures are typed — they fail this Paylocity attempt only. DROP results live
on matching-dev and are not touched.

Never logs email, hashes, or vendor ids. ``source_matching_attempt_id`` stays
``None`` — the snapshot FK is ``matching_attempts``, not ``paylocity_attempts``.
"""
...
ADAPTER = "paylocity_hash"
PAYLOCITY_VERTICAL = "paylocity"
...
class PaylocityHashLookupError(Exception):
    """Paylocity mart lookup failed in a way that should retry (timeout / transport)."""
...
class VerticalMatchOutcome:
    """Count-only result of one Paylocity matching attempt. No hashes or vendor ids."""
...
def _error_code(exc: BaseException) -> str:
    ...
    if isinstance(exc, ValueError) and "plaintext" in str(exc).lower():
        return "paylocity_invalid_hash"
    return "paylocity_lookup_error"


def _failure_outcome(exc: BaseException) -> VerticalMatchOutcome:
    code = _error_code(exc)
    logger.error(
        "paylocity_vertical_match_failed",
        extra={
            "event": "paylocity_vertical_match_failed",
            "error_code": code,
...
async def _persist_snapshot(
    ...
    *,
    ...
    # Paylocity provenance lives on paylocity_attempts. Do not write
    # paylocity_attempts.id into source_matching_attempt_id (FK to matching_attempts).
    await persist(
        conn,
...
def _query_job_config(hash_value: str) -> Any:
    try:
        ...
        raise PaylocityHashLookupError("google-cloud-bigquery is not installed") from exc
...
def _default_client() -> Any:
    try:
        ...
        raise PaylocityHashLookupError("google-cloud-bigquery is not installed") from None
...
def lookup_paylocity_vendor_ids_by_email_hash(
    ...
    *,
    ...
    """Return opaque Paylocity vendor_record_id values for one email hash.

    Queries ``{project}.external_hash_index.paylocity_email_hash__build`` with
    ``system = 'paylocity'``. Env overrides: ``EXTERNAL_HASH_BQ_PROJECT``,
    ...
    logger.info(
        "paylocity hash lookup complete",
        extra={
...
def _default_lookup(kind: str, hash_value: str) -> list[str]:
    if kind == "email":
        return lookup_paylocity_vendor_ids_by_email_hash(hash_value)
    if kind == "phone":
        return lookup_vendor_ids_by_phone_hashes(
            [hash_value],
            table=PAYLOCITY_PHONE_HASH_BUILD_TABLE,
            system="paylocity",
        ).get(hash_value, [])
    return lookup_vendor_ids_by_ndz_hashes(
        [hash_value],
        table=PAYLOCITY_NDZ_HASH_BUILD_TABLE,
        system="paylocity",
    ).get(hash_value, [])


async def run_paylocity_vertical_match(
    conn: Any,
    *,
    request_id: str,
    attempt_id: int,
    ...
    """Look up Paylocity vendor ids for one claimed ``paylocity_attempts`` matching row.
    ...
    ``PaylocityHashLookupError`` and opaque-hash ``ValueError`` persist nothing
    and return a typed failure.
    """
    del attempt_id
    ...
    if not hash_value or kind is None:
        try:
            ...
            return _failure_outcome(exc)
        logger.info(
            "paylocity_vertical_match_recorded",
            extra={
                "event": "paylocity_vertical_match_recorded",
                "match_count": 0,
            },
        )
    ...
    except (PaylocityHashLookupError, ValueError) as exc:
        return _failure_outcome(exc)
    except Exception as exc:
        return _failure_outcome(exc)
