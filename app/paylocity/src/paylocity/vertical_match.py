...
ADR-21), looks up opaque vendor ids on ``paylocity_email_hash__build``, and
upserts ``request_vertical_matching``. Lookup failures are typed — they fail
this Paylocity attempt only. DROP results live on matching-dev and are not
touched.
...
async def run_paylocity_vertical_match(
    ...
    *,
    ...
    zero-hit snapshot and succeeds. ``PaylocityHashLookupError`` and plaintext-``@``
    ...
    try:
        lookup_fn = lookup or lookup_paylocity_vendor_ids_by_email_hash
