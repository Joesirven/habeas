"""Mailchimp Marketing API error classification."""
...
class MailchimpErrorClassifier:
    """Map Mailchimp error codes to shared queue dispositions."""

    terminal_success = frozenset({"not_found", "already_unsubscribed", "invalid_email"})
...
def classify_mailchimp_error(
    error_code: str,
    error_payload: dict[str, Any] | None = None,
) -> TerminalAction:
    """Classify a Mailchimp error and return the terminal workflow action."""
    return classify_with(MailchimpErrorClassifier(), error_code, error_payload)
