"""Hash refresh route — mapped upload extract, optional dbt, no PII leak."""
...
from lever.hash_extract import HashExtractError
from lever.main import DbtRunResult
...
def _hash_refresh_mocks(*, claim: dict | None, extract, dbt):
    ...
    with ExitStack() as stack:
        get_pool = stack.enter_context(patch("lever.main.get_pool"))
        ...
        stack.enter_context(patch("lever.main.run_hash_extract", extract))
        stack.enter_context(patch("lever.main.run_external_hash_dbt_build", dbt))
...
def test_hash_refresh_in_flight_when_self_enqueue_still_unclaimable(client):
    ...
    with _hash_refresh_mocks(claim=None, extract=extract, dbt=dbt) as mocks:
        with patch(
            "lever.main.enqueue_vertical_hash_refresh",
            new_callable=AsyncMock,
            return_value=9,
        ) as enqueue:
    ...
    assert enqueue.await_args.kwargs["system"] == "lever"
    assert mocks.claim.await_count == 2
    assert mocks.claim.await_args.kwargs["system"] == "lever"
...
def test_hash_refresh_success_records_extract_rows(client):
    ...
    assert extract.await_args.kwargs["bq_table"] == "lever_hashed_raw"
    dbt.assert_called_once()
