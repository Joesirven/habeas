"""Lever chunk drain — claims lever_attempts only; lease_key='lever'; set-based BQ."""
...
from lever.bq_lookup import LeverHashLookupError
from lever.chunk_drain import (
    DEFAULT_DRAIN_LEASE_HOLDER,
    LEVER_LEASE_KEY,
    claim_lever_matching_chunk,
    drain_task_count,
    ensure_drain,
    job_task_worker_id,
    process_lever_matching_chunk,
    run_job_task,
...
_VENDOR_ID = "lever-opaque-must-not-log"
...
def _gate_allowed() -> GateResult:
    return GateResult(allowed=True, code="ok", display_status="ready")


def _gate_blocked() -> GateResult:
    return GateResult(
        allowed=False, code="stale_upload", display_status="blocked", blocking_system="lever"
    )
...
@pytest.mark.asyncio
async def test_claim_uses_lever_attempts_skip_locked() -> None:
    conn = _RecordingConn(
        fetch_rows=[{"id": 3, "request_id": _REQUEST_ID, "attempt_number": 1}]
    )

    rows = await claim_lever_matching_chunk(conn, worker_id="lever-matching-drain")

    assert len(rows) == 1
    sql = _all_sql(conn)
    assert "lever_attempts" in sql
...
async def test_process_chunk_idle_when_empty() -> None:
    ...
    out = await process_lever_matching_chunk(conn, worker_id="w1")
...
@pytest.mark.asyncio
async def test_process_chunk_gate_blocked_marks_submit_error() -> None:
    conn = _RecordingConn(
        fetch_rows=[{"id": 15, "request_id": _REQUEST_ID, "attempt_number": 1}]
    )
    lookup = MagicMock()
    ...
    with patch(
        "lever.chunk_drain.evaluate_vertical_matching_gate",
        new_callable=AsyncMock,
        return_value=_gate_blocked(),
    ):
        out = await process_lever_matching_chunk(
            conn, worker_id="w1", lookup_batch=lookup
        )
...
async def test_process_chunk_lookup_error_marks_submit_error_with_retry() -> None:
    ...
    def _lookup(_hashes: list[str]) -> dict[str, list[str]]:
        raise LeverHashLookupError("timeout", retry_seconds=120)
...
async def test_ensure_drain_idle_when_no_pending() -> None:
    ...
    with patch("lever.chunk_drain.acquire_drain_lease", acquire):
...
@pytest.mark.asyncio
async def test_ensure_drain_acquires_lever_lease_and_starts_job() -> None:
    ...
    with (
        patch("lever.chunk_drain.acquire_drain_lease", acquire),
        patch("lever.chunk_drain.renew_drain_lease", renew),
    ):
    ...
    assert out["lease_key"] == LEVER_LEASE_KEY
    assert started is True
    assert acquire.await_args.kwargs["lease_key"] == LEVER_LEASE_KEY
...
async def test_ensure_drain_releases_lease_when_job_start_fails() -> None:
    ...
    with (
        patch(
            "lever.chunk_drain.acquire_drain_lease", AsyncMock(return_value=True)
        ),
        patch("lever.chunk_drain.release_drain_lease", release),
    ):
        out = await ensure_drain(
            conn, holder=DEFAULT_DRAIN_LEASE_HOLDER, start_job=_start
        )
    ...
    assert release.await_args.kwargs["lease_key"] == LEVER_LEASE_KEY
...
async def test_ensure_drain_reports_active_when_lease_held() -> None:
    ...
    with patch(
        "lever.chunk_drain.acquire_drain_lease", AsyncMock(return_value=False)
    ):
        out = await ensure_drain(conn, holder=DEFAULT_DRAIN_LEASE_HOLDER)
...
async def test_run_job_task_drains_until_empty_and_releases() -> None:
    ...
    with (
        patch(
            "lever.chunk_drain.process_lever_matching_chunk",
            new_callable=AsyncMock,
            side_effect=_process,
        ),
        patch(
            "lever.chunk_drain.renew_drain_lease", AsyncMock(return_value=True)
        ),
        patch("lever.chunk_drain.release_drain_lease", release),
    ):
        out = await run_job_task(conn, worker_id="lever-matching-drain-task0")
...
def test_job_task_worker_id_includes_task_index(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("WORKER_ID", "lever-matching-drain")
    monkeypatch.setenv("CLOUD_RUN_TASK_INDEX", "3")
    assert job_task_worker_id() == "lever-matching-drain-task3"


def test_drain_task_count_defaults_and_override(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("LEVER_DRAIN_TASK_COUNT", raising=False)
    assert drain_task_count() == 5
    monkeypatch.setenv("LEVER_DRAIN_TASK_COUNT", "20")
    assert drain_task_count() == 20
    monkeypatch.setenv("LEVER_DRAIN_TASK_COUNT", "junk")
    assert drain_task_count() == 5
...
async def test_start_drain_job_execution_posts_run(
    ...
    monkeypatch.setenv("LEVER_DRAIN_JOB_NAME", "lever-matching-drain-prod")
    monkeypatch.setenv("LEVER_DRAIN_JOB_REGION", "us-east4")
    monkeypatch.setenv("LEVER_DRAIN_TASK_COUNT", "5")
    ...
    assert out["job_name"] == "lever-matching-drain-prod"
    assert out["execution"] == "executions/xyz"
    assert out["task_count"] == 5
    client.post.assert_awaited_once()
    url = client.post.await_args.args[0]
    assert url.endswith("/jobs/lever-matching-drain-prod:run")
...
async def test_start_drain_job_execution_requires_job_name(
    ...
    monkeypatch.delenv("LEVER_DRAIN_JOB_NAME", raising=False)
    with pytest.raises(RuntimeError, match="LEVER_DRAIN_JOB_NAME"):
...
async def test_process_chunk_logs_no_pii(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with (
        ...
        caplog.at_level(logging.INFO, logger="lever.chunk_drain"),
    ):
        await process_lever_matching_chunk(
            conn,
            worker_id="w1",
            lookup_batch=_lookup,
            persist=AsyncMock(),
    ...
    blob = caplog.text
    assert "lever_chunk_completed" in blob
