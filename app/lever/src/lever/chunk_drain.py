...
def _mart_table_for_kind(kind: str) -> str | None:
    if kind == "email":
        return LEVER_EMAIL_HASH_BUILD_TABLE
    if kind == "phone":
        return LEVER_PHONE_HASH_BUILD_TABLE
    if kind == "ndz":
        return LEVER_NDZ_HASH_BUILD_TABLE
    return None
...
def _lookup_hashes_for_kind(
    ...
    *,
    ...
    if kind == "email":
        return lookup_lever_vendor_ids_by_email_hashes(hash_values, client=bq_client)
    if kind == "phone":
        ...
        for kind, group in by_kind.items():
            if lookup_batch is None:
                try:
                    ...
                    safe = redact_error_text(str(exc))
                    logger.error(
                        "lever_drain_mart_check_error",
                        extra={
                            "event": "lever_drain_mart_check_error",
                            "list_kind": kind,
                            "error_summary": safe,
                        },
                    )
                if not mart_ok:
                        ...
                        outcomes.append(
                            {
                                ...
                                "audit_payload": _error_audit(
                                    error_code="lever_lookup_error",
                                    error_class="LeverHashLookupError",
                                    error_detail=(
                    ...
                    logger.info(
                        "lever_drain_mart_missing",
                        extra={
                            "event": "lever_drain_mart_missing",
                            "list_kind": kind,
                            "prepared": len(group),
                        },
                    )
