...
from drop_notice_dispatcher.dispatch import run_weekly_amend, run_weekly_upload
...
@app.post("/amend-weekly")
async def amend_weekly(body: UploadWeeklyRequest | None = None):
    """Amend Id,Status for previously uploaded rows whose status changed.

    Intended schedule: Wednesday 04:00 America/Los_Angeles (hash-refresh rematch).
    """
    if not settings.database_url:
        raise HTTPException(status_code=503, detail="DATABASE_URL not configured")
    if not settings.drop_connector_url:
    ...
    async with pool.acquire() as conn:
        try:
            ...
            logger.exception(
                "amend_weekly_failed",
                extra={"event": "amend_weekly_failed"},
            )
            raise HTTPException(status_code=500, detail="amend-weekly failed") from None

    return {
        "status": "ok" if result.uploaded else "idle",
        "amended": result.uploaded,
        "skipped": result.skipped,
        "failed": result.failed,
        "batches": [
            {
