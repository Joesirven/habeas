...
async def find_ready_rows(
    ...
    *,
    ...
    """DROP rows ready for weekly upload (notice.review approved, response_status set)."""
    rows = await conn.fetch(
        ...
        SELECT r.id::text AS request_id,
         ...
         WHERE r.intake_source = 'drop'
           ...
           AND NOT EXISTS (
                 SELECT 1
                   FROM drop_response_submissions drs
                  WHERE drs.source_csv_filename = drr.source_csv_filename
                    AND drs.submission_type = 'upload'
               )
