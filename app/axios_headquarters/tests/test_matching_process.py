"""Mart-based AxiosHeadquarters matching — lookup, snapshot, complete; no PII in audit."""
...
from habeas_privacy_core.queue.constants import AXIOS_HEADQUARTERS_ATTEMPTS_TABLE, STEP_MATCHING
...
_AUDIT_FORBIDDEN_KEYS = frozenset(
    {
        ...
        "vendor_record_id",
        "vendor_record_ids",
        "axios_headquarters_employee_id",
        "matched_external_id",
    }
)
...
async def _run_process(
    *,
    ...
    outcome = await run_axios_headquarters_vertical_match(
        MagicMock(),
...
async def test_lookup_error_fails_and_persists_nothing():
    lookup_fn = MagicMock(side_effect=AxiosHeadquartersHashLookupError("timeout", retry_seconds=120))
    persist = AsyncMock()
    ...
    assert outcome.error_code == "axios_headquarters_lookup_error"
    assert outcome.match_count == 0
...
def test_matching_submit_email_hash_looks_up_upserts_and_completes(client):
    ...
    with (
        patch("axios_headquarters.main.claim_next", new_callable=AsyncMock, return_value=claim_row),
        patch("axios_headquarters.main.get_pool") as get_pool,
        patch(
            "axios_headquarters.main.evaluate_vertical_matching_gate",
        ...
        patch("axios_headquarters.main._complete_stub", new_callable=AsyncMock) as complete_stub,
    ):
    ...
    assert AXIOS_HEADQUARTERS_ATTEMPTS_TABLE in bind.sql
...
def test_matching_submit_audit_payload_has_no_pii(client, caplog):
    ...
    body = response.json()
    bind = _complete_bind(conn)
    assert bind.audit["adapter"] == "axios_headquarters_hash"
    assert bind.audit["step"] == STEP_MATCHING
    assert bind.audit["system"] == "axios_headquarters"
    _assert_no_pii(bind.audit)
    _assert_no_pii(body)
...
async def test_load_path_uses_drop_email_hash_fields():
    ...
    with (
        patch("axios_headquarters.vertical_match.get_request", new_callable=AsyncMock, return_value=record),
        patch(
            "axios_headquarters.vertical_match.request_resolver",
            new_callable=AsyncMock,
