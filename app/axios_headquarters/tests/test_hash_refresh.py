...
from axios_headquarters.hash_extract import HashExtractError
from axios_headquarters.main import DbtRunResult
...
def client(monkeypatch):
    from axios_headquarters import main
...
def _hash_refresh_mocks(*, claim: dict | None, extract, dbt):
    ...
    with ExitStack() as stack:
        get_pool = stack.enter_context(patch("axios_headquarters.main.get_pool"))
        claim_refresh = stack.enter_context(
            patch(
                "axios_headquarters.main.claim_vertical_hash_refresh",
        ...
        mark = stack.enter_context(
            patch(
                "axios_headquarters.main.mark_vertical_hash_refresh_in_flight",
                new_callable=AsyncMock,
            )
        )
        record = stack.enter_context(
            patch(
                "axios_headquarters.main.record_vertical_hash_refresh_run",
                new_callable=AsyncMock,
                return_value=1,
            )
        )
        stack.enter_context(patch("axios_headquarters.main.run_hash_extract", extract))
        stack.enter_context(patch("axios_headquarters.main.run_external_hash_dbt_build", dbt))
...
def test_hash_refresh_in_flight_when_self_enqueue_still_unclaimable(client):
    ...
    with _hash_refresh_mocks(claim=None, extract=extract, dbt=dbt) as mocks:
        with patch(
            "axios_headquarters.main.enqueue_vertical_hash_refresh",
            new_callable=AsyncMock,
            return_value=9,
        ) as enqueue:
    ...
    assert enqueue.await_args.kwargs["system"] == "axios_headquarters"
...
def test_hash_refresh_self_enqueues_then_processes(client):
    ...
    claim_row = {"id": 9, "system": "axios_headquarters"}
    with _hash_refresh_mocks(claim=claim_row, extract=extract, dbt=dbt) as mocks:
        mocks.claim.side_effect = [None, claim_row]
...
def test_hash_refresh_success_records_extract_rows(client):
    ...
    with _hash_refresh_mocks(
        claim={"id": 99, "system": "axios_headquarters"},
        extract=extract,
        dbt=dbt,
    ) as mocks:
        response = client.post("/hash-refresh/process")
    ...
    assert extract.await_args.kwargs["bq_table"] == "axios_headquarters_hashed_raw"
    dbt.assert_called_once()
...
def test_hash_refresh_skips_dbt_when_configured(client, monkeypatch):
    ...
    with _hash_refresh_mocks(
        claim={"id": 7, "system": "axios_headquarters"},
        extract=extract,
        dbt=dbt,
    ) as mocks:
        response = client.post("/hash-refresh/process")
...
def test_hash_refresh_extract_failure_does_not_mark_success(client, caplog):
    ...
    with (
        ...
        _hash_refresh_mocks(
            claim={"id": 11, "system": "axios_headquarters"},
            extract=extract,
            dbt=dbt,
        ) as mocks,
    ):
...
def test_hash_refresh_empty_extract_does_not_mark_success(client):
    ...
    with _hash_refresh_mocks(
        claim={"id": 31, "system": "axios_headquarters"},
        extract=extract,
        dbt=dbt,
    ) as mocks:
        response = client.post("/hash-refresh/process")
...
def test_hash_refresh_empty_hash_extract_error_does_not_mark_success(client):
    ...
    with _hash_refresh_mocks(
        claim={"id": 32, "system": "axios_headquarters"},
        extract=extract,
        dbt=dbt,
    ) as mocks:
        response = client.post("/hash-refresh/process")
...
def test_hash_refresh_dbt_failure_does_not_mark_success(client):
    ...
    with _hash_refresh_mocks(
        claim={"id": 13, "system": "axios_headquarters"},
        extract=extract,
        dbt=dbt,
    ) as mocks:
        response = client.post("/hash-refresh/process")
