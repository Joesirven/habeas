...
from axios_headquarters.hash_extract import (
    DEFAULT_BQ_TABLE,
    SYSTEM,
    HashExtractError,
    run_hash_extract,
...
HASH_1 = "hashed-one"
HASH_2 = "hashed-two"
GCS_URI = "gs://uploads/connections/axios_headquarters/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee/upload.csv"
...
async def test_run_hash_extract_writes_hashed_rows_not_raw_email():
    ...
    assert reader.last == (
        "uploads",
        "connections/axios_headquarters/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee/upload.csv",
    )
    ...
    assert all(row["system"] == SYSTEM == "axios_headquarters" for row in payloads)
