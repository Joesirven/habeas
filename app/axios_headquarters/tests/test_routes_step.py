...
from axios_headquarters.vertical_match import VerticalMatchOutcome
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            assert evaluate_gate.await_args.kwargs["system"] == "axios_headquarters"
                            ...
                            assert table == "axios_headquarters_attempts"
...
async def test_complete_gate_blocked_writes_allowlisted_audit():
    ...
    assert audit["system"] == "axios_headquarters"
    assert audit["gate_code"] == "rotation_overdue"
    assert audit["display_status"] == "action_required"
    assert audit["error_code"] == "gate_blocked"


def test_suppression_submit_respects_suppress_axios_headquarters_approval_gate():
    """Without suppress.axios_headquarters approval, suppression submit must not complete."""
    from axios_headquarters import main
    ...
    active_rule = {
        "id": 1,
        "action_type": "suppress.axios_headquarters",
    ...
    try:
        with patch.object(main, "claim_next", new_callable=AsyncMock) as claim_next:
            with patch.object(main, "get_pool", return_value=_FakePool()):
                with patch.object(main, "fetch_active_rule", new_callable=AsyncMock) as fetch_rule:
                    with patch.object(
                        main,
                        "_is_suppress_axios_headquarters_approved",
                        new_callable=AsyncMock,
                    ) as is_approved:
                        with patch.object(
                            main,
        ...
        assert fetch_rule.await_args.args[1] == "suppress.axios_headquarters"
...
def test_suppression_submit_fail_closed_when_rule_missing():
    """Missing suppress.axios_headquarters rule must not allow suppression to complete."""
    from axios_headquarters import main
