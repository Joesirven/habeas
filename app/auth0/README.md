...
Cloud Run FastAPI app — health + step routes. Matching (`POST /matching/submit`) looks up Auth0 email / phone / NDZ marts by DROP list type; suppression still uses `adapters/stub.py`. Hash refresh runs the Auth0 **users-export job**, hashes email and `phone_number` in memory (nullable `email_hash` / `phone_hash`; `ndz_hash` stays null — Management API export has no NDZ parts), writes hashed raw, then runs dbt. Depends on [`habeas-privacy-core`](../../libs/habeas-privacy-core/).
...
2. Run the Auth0 Management API **users-export job** (`POST /api/v2/jobs/users-exports`, poll job, download gzipped NDJSON). Required scope `read:users`. Hash email and Auth0 profile `phone_number` in memory (DROP v1.2.0 / CPPA). Skip users without a hashable email **or** phone. `ndz_hash` is **not** populated from this export (no name/DOB/ZIP fields). This is a full export — not `GET /api/v2/users` paging. Optional `AUTH0_USERS_MAX_PAGES` is a test-only yield cap that **aborts** with `export_incomplete` (not page/per_page).
3. Write `example-gcp-project.external_hash_index.auth0_hashed_raw` with hashed columns only: nullable `email_hash`, `phone_hash`, `ndz_hash` (always null for Auth0), plus `vendor_record_id`, `system`, `extracted_at`. Empty incoming rows must not wipe a prior good index — empty `WRITE_TRUNCATE` is being removed in the hashed-raw writer (do not treat `rows_written=0` as a successful replace).
4. Run `dbt build --select stg_auth0_hashed mart_auth0_email_hash mart_auth0_phone_hash mart_auth0_ndz_hash` from [`transform/external_hash`](../../transform/external_hash/) (physical aliases `auth0_email_hash__build`, `auth0_phone_hash__build`, `auth0_ndz_hash__build`).

dbt never sees plaintext email or phone. Do not log emails, phones, `client_secret`, download URLs, or raw Management API payloads.
...
# (`CREATE TABLE … auth0_hashed_raw` — nullable email_hash / phone_hash /
#  ndz_hash + vendor ids only; no plaintext).
```
...
Do **not** treat dataset-wide `dataEditor` on `external_hash_index` as acceptable: that dataset also holds other vertical hashed raw / marts. Bind table-level editor on `auth0_hashed_raw` and the Auth0 email / phone / NDZ `__build` marts only, plus project `jobUser` and GSM `secretAccessor` on `dpra-connections-auth0-{connection_id}`.

## Auth0 matching on dev

Email / Phone / NDZ matching on **dev** runs in **this worker** (`POST /matching/submit`), not `matching-dev`. **matching-dev is DROP-only** — it does not look up the Auth0 marts, does not write `request_vertical_matching`, and must **not** be granted `auth0_hashed_raw` (or dataset-wide `external_hash_index`).

Admin-api is the control plane: super_admin enqueue/process matching (S05). Users never invoke workers. The serving builds (`auth0_email_hash__build`, `auth0_phone_hash__build`, `auth0_ndz_hash__build` in `example-gcp-project.external_hash_index`) must exist before this worker looks them up. Auth0 extract leaves `ndz_hash` null, so NDZ lookups against an empty NDZ mart yield `match_count=0`.
...
Empty mart → matching records `match_count=0`. Confirm dbt built the email / phone / NDZ `__build` tables (Jose-gated BQ row count) before blaming matching.
...
This worker claims the pending `auth0_attempts` row, looks up the Auth0 email / phone / NDZ `__build` mart for the request’s DROP list type (Email → `auth0_email_hash__build`, Phone → `auth0_phone_hash__build`, NDZ → `auth0_ndz_hash__build`), and upserts `request_vertical_matching`. Missing DROP hash for that list type → zero-hit snapshot (not an error). Empty mart → `match_count=0`.
