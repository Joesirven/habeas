"""Mart-based Auth0 matching — lookup, snapshot, complete; no PII in audit."""
...
import pytest
from auth0.vertical_match import VerticalMatchOutcome, run_auth0_vertical_match
...
async def test_lookup_error_fails_and_persists_nothing():
    lookup_fn = MagicMock(side_effect=Auth0HashLookupError("timeout", retry_seconds=120))
    persist = AsyncMock()
...
def test_matching_submit_email_hash_looks_up_upserts_and_completes(client):
    ...
    with (
        ...
        patch("auth0.adapters.stub.StubMatchAdapter") as stub_match,
    ):
    ...
    assert AUTH0_ATTEMPTS_TABLE in bind.sql
...
def test_matching_submit_lookup_error_fails_claimed_row(client):
    test_client, _main = client
    claim_row = {"id": _ATTEMPT_ID, "request_id": _REQUEST_ID}
    conn = AsyncMock()
    persist = AsyncMock()
    ...
    assert body["reason"] == "auth0_lookup_error"
    persist.assert_not_awaited()
    bind = _complete_bind(conn)
    assert bind.attempt_id == _ATTEMPT_ID
    assert bind.status == "submit_error"
    assert bind.status != "claimed"
    assert AUTH0_ATTEMPTS_TABLE in bind.sql
    assert "claimed" in bind.sql
    assert bind.error_code == "auth0_lookup_error"
    _assert_no_pii(bind.audit)
    _assert_no_pii(body)
...
def test_matching_submit_audit_payload_has_no_pii(client, caplog):
    ...
    body = response.json()
    bind = _complete_bind(conn)
    assert bind.audit["adapter"] == "auth0_hash"
    assert bind.audit["step"] == STEP_MATCHING
    assert bind.audit["system"] == "auth0"
    _assert_no_pii(bind.audit)
    _assert_no_pii(body)
