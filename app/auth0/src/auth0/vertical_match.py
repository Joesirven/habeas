...
ADR-21), looks up opaque vendor ids on ``auth0_email_hash__build``, and
upserts ``request_vertical_matching``. Lookup failures are typed — they fail
this Auth0 attempt only. DROP results live on matching-dev and are not touched.
...
async def run_auth0_vertical_match(
    ...
    *,
    ...
    zero-hit snapshot and succeeds. ``Auth0HashLookupError`` and plaintext-``@``
    ...
    try:
        lookup_fn = lookup or lookup_auth0_vendor_ids_by_email_hash
