"""Auth0 worker matching — mart lookup + snapshot persist.

Loads the request's DROP hash for Email / Phone / NDZ (same field order as
matching/drop_hash ADR-21), looks up opaque vendor ids on the matching Auth0
serving mart, and upserts ``request_vertical_matching``. Lookup failures are
typed — they fail this Auth0 attempt only. DROP results live on matching-dev
and are not touched.
...
ListKind = Literal["email", "phone", "ndz"]

_HASH_LABEL_BY_KIND: dict[ListKind, str] = {
    "email": "email_hash",
    "phone": "phone_hash",
    "ndz": "ndz_hash",
}
...
__all__ = [
    ...
    "run_auth0_vertical_match",
]


@dataclass(frozen=True, slots=True)
class VerticalMatchOutcome:
    """Count-only result of one Auth0 matching attempt. No hashes or vendor ids."""
...
def _list_kind(list_type: DropListType | str | None) -> ListKind | None:
    normalized = normalize_drop_list_type(list_type)
    if normalized is None:
        return None
    return normalized.name.lower()  # type: ignore[return-value]
...
async def _persist_snapshot(
    ...
    *,
    ...
    # Auth0 provenance lives on auth0_attempts. Do not write auth0_attempts.id
...
def _default_lookup(kind: ListKind, hash_value: str) -> list[str]:
    if kind == "email":
        return lookup_auth0_vendor_ids_by_email_hash(hash_value)
    try:
        if kind == "phone":
...
async def run_auth0_vertical_match(
    ...
    *,
    ...
    """Look up Auth0 vendor ids for one claimed ``auth0_attempts`` matching row.
    ...
    ``Auth0HashLookupError`` and opaque-hash ``ValueError`` persist nothing
    and return a typed failure.
    """
    del attempt_id  # reserved for audit correlation; not written to snapshot FK
