...
- Deployed admin-api requires **Identity-Aware Proxy** identity (`REQUIRE_IAP_IDENTITY=true`); CLI uses `IAP_OAUTH_CLIENT_ID` / `IAP_ID_TOKEN` — see `infra/README.md`. Never call workers directly.
